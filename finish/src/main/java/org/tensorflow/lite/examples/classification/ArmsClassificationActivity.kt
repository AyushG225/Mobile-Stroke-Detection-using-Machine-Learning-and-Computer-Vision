...
import org.tensorflow.lite.examples.classification.camera.CameraSource
import org.tensorflow.lite.examples.classification.data.Device
import org.tensorflow.lite.examples.classification.ml.*
import java.util.*

@ExperimentalCoroutinesApi
class ArmsClassificationActivity : AppCompatActivity() {
    companion object {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        println("in oncreate of armsclassification")
        super.onCreate(savedInstanceState)
        setContentView(R.layout.arms_classification)
    ...
    override fun onStart() {
        ...
        timer?.schedule(object: TimerTask(){
            override fun run() {
                changescreen = Intent(this@ArmsClassificationActivity, ScreenAfterArmsActivity::class.java)
                changescreen.putExtra("leftcnt", (cameraSource?.leftcnt).toString())
                changescreen.putExtra("rightcnt", cameraSource?.rightcnt.toString())
                startActivity(changescreen)
            }
        }, 10000)
        openCamera()
        cameraSource?.let { println("leftcnt in armsclassification onstart is " + it.leftcnt)
            println("rightcnt in armsclassification onstart is " + it.rightcnt) }
    }
    ...
    override fun onPause() {
        cameraSource?.let { println("leftcnt in armsclassification onpause is " + it.leftcnt)
            println("rightcnt in armsclassification onpause is " + it.rightcnt) }
        cameraSource?.close()
        cameraSource = null
        super.onPause()
    }
    ...
    private fun openCamera() {
        println("inopencamera, iscamerapermissiongranted = " + isCameraPermissionGranted())
        if (isCameraPermissionGranted()) {
            if (cameraSource == null) {
                println("inopencamera2")
                cameraSource =
                    CameraSource(surfaceView, object : CameraSource.CameraSourceListener {
                        ...
                        println("inopencamera3")
                        prepareCamera()
                    }
                isPoseClassifier()
                lifecycleScope.launch(Dispatchers.Main) {
                    println("inopencamera4")
                    cameraSource?.initCamera()
                }
            }
            println("inopencamera5")
            createPoseEstimator()
        }
        cameraSource?.let { println("leftcnt in armsclassification opencamera is " + it.leftcnt)
            println("rightcnt in armsclassification opencamera is " + it.rightcnt) }
    }
